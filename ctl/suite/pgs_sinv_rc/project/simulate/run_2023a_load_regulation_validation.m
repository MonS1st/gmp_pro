function results = run_2023a_load_regulation_validation( ...
        stop_time, target_current_a)
%RUN_2023A_LOAD_REGULATION_VALIDATION Validate the 0-2 A load regulation.
%
% The sweep uses PGS_2023A_SINGLE_INV_RLOAD_2022b and the existing Debug/x64
% SIL executable. The default five points retain the formal output names;
% any valid subset containing 0 A and 2 A writes only *_quick artifacts.
% Every load point uses an isolated SimulationInput variable override and a
% separately launched controller process. The loaded model is never saved.
...
default_target_current_a = [0, 0.5, 1.0, 1.5, 2.0];
target_current_a = validate_target_current_vector(target_current_a);
is_full_sweep = isequal(target_current_a, default_target_current_a);
approximate_no_load = target_current_a == 0.0;
load_resistance_ohm = zeros(size(target_current_a));
load_resistance_ohm(approximate_no_load) = 1e6;
load_resistance_ohm(~approximate_no_load) = ...
    target_voltage_rms_v ./ target_current_a(~approximate_no_load);
...
try
    ...
    load_current_feedforward_enabled = logical( ...
        config.SINV_2023A_ENABLE_LOAD_CURRENT_FF);
    load_current_feedforward_gain = ...
        config.SINV_2023A_LOAD_CURRENT_FF_GAIN;
    capacitor_current_feedforward_enabled = logical( ...
        config.SINV_2023A_ENABLE_CAP_CURRENT_FF);
    capacitor_current_feedforward_gain = ...
        config.SINV_2023A_CAP_CURRENT_FF_GAIN;
    ...
    fprintf('  Load-current feedforward: %s\n', ...
        enabled_text(load_current_feedforward_enabled));
    fprintf('  Load-current FF gain: %.9g\n', ...
        load_current_feedforward_gain);
    fprintf('  Capacitor-current feedforward: %s\n', ...
        enabled_text(capacitor_current_feedforward_enabled));
    fprintf('  Capacitor-current FF gain: %.9g\n', ...
        capacitor_current_feedforward_gain);
    ...
    results = struct( ...
        ...
        'deadtime_compensation_enabled', ...
            deadtime_compensation_enabled, ...
        'load_current_feedforward_enabled', ...
            load_current_feedforward_enabled, ...
        'load_current_feedforward_gain', ...
            load_current_feedforward_gain, ...
        'capacitor_current_feedforward_enabled', ...
            capacitor_current_feedforward_enabled, ...
        'capacitor_current_feedforward_gain', ...
            capacitor_current_feedforward_gain, ...
    ...
    json_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_metrics%s.json', artifact_suffix));
    csv_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_points%s.csv', artifact_suffix));
    png_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_curve%s.png', artifact_suffix));
    harmonics_csv_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_harmonics%s.csv', artifact_suffix));
    harmonic_diagnostics_png_file = fullfile(result_dir, sprintf( ...
        ['2023a_load_regulation_harmonic_diagnostics' ...
         '%s.png'], artifact_suffix));
...
function target_current_a = validate_target_current_vector( ...
        target_current_a)
if ~isnumeric(target_current_a) || ~isreal(target_current_a) || ...
        ~isvector(target_current_a) || isempty(target_current_a) || ...
        any(~isfinite(target_current_a(:)))
    error('SINV:R2023A:LoadRegulationTargetCurrentInvalid', ...
        'Target currents must be a finite real numeric vector.');
end
target_current_a = double(target_current_a(:)');
if any(target_current_a < 0.0) || any(target_current_a > 2.0)
    error('SINV:R2023A:LoadRegulationTargetCurrentOutOfRange', ...
        'Target currents must remain within 0 A through 2 A.');
end
if any(diff(target_current_a) <= 0.0)
    error('SINV:R2023A:LoadRegulationTargetCurrentOrderInvalid', ...
        'Target currents must be strictly increasing and nonrepeating.');
end
if target_current_a(1) ~= 0.0 || target_current_a(end) ~= 2.0
    error('SINV:R2023A:LoadRegulationEndpointMissing', ...
        ['Target currents must include both 0 A and 2 A so endpoint ' ...
         'load regulation remains defined.']);
end
end
...
names = {
    ...
    'SINV_2023A_ENABLE_LOAD_CURRENT_FF'
    'SINV_2023A_LOAD_CURRENT_FF_GAIN'
    'SINV_2023A_ENABLE_CAP_CURRENT_FF'
...
if config.SINV_2023A_ENABLE_LOAD_CURRENT_FF ~= 0 && ...
        config.SINV_2023A_ENABLE_LOAD_CURRENT_FF ~= 1
    error('SINV:R2023A:LoadRegulationLoadCurrentFFInvalid', ...
        'SINV_2023A_ENABLE_LOAD_CURRENT_FF must be 0 or 1.');
end
if config.SINV_2023A_ENABLE_CAP_CURRENT_FF ~= 0 && ...
        config.SINV_2023A_ENABLE_CAP_CURRENT_FF ~= 1
    error('SINV:R2023A:LoadRegulationCapCurrentFFInvalid', ...
        'SINV_2023A_ENABLE_CAP_CURRENT_FF must be 0 or 1.');
end
end
