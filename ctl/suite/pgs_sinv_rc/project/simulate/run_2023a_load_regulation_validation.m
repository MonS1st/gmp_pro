...
common_header_file = fullfile(root, '..', '..', 'src', ...
    'sdpe_pgs_sinv_rc_common_settings.h');
common_matlab_init_file = fullfile(root, '..', '..', 'sdpe_general', ...
...
try
    ...
    config = assert_generated_configuration( ...
        header_file, matlab_init_file, ...
        common_header_file, common_matlab_init_file, ...
    ...
    deadtime_compensation_enabled = logical( ...
        config.SINV_2023A_ENABLE_DEADTIME_COMP);
    current_direction_deadband_a = ...
        config.CTRL_CURRENT_DB_PU * config.CTRL_CURRENT_BASE;
    ...
    fprintf('  Dead-time compensation: %s\n', ...
        enabled_text(deadtime_compensation_enabled));
    fprintf('  PWM deadband compensation: %.9g timer ticks\n', ...
        config.CTRL_PWM_DEADBAND_CMP);
    fprintf('  Current-direction deadband: %.9g pu\n', ...
        config.CTRL_CURRENT_DB_PU);
    fprintf('  Current-direction deadband: %.9g A peak\n', ...
        current_direction_deadband_a);
    ...
    results = struct( ...
        ...
        'deadtime_compensation_enabled', ...
            deadtime_compensation_enabled, ...
        'pwm_deadband_compensation_ticks', ...
            config.CTRL_PWM_DEADBAND_CMP, ...
        'current_direction_deadband_pu', ...
            config.CTRL_CURRENT_DB_PU, ...
        'current_direction_deadband_a', ...
            current_direction_deadband_a, ...
    ...
    if deadtime_compensation_enabled
        state_suffix = 'dbcomp_on';
    else
        state_suffix = 'dbcomp_off';
    end
    state_json_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_metrics_%s.json', state_suffix));
    state_csv_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_points_%s.csv', state_suffix));
    state_harmonics_csv_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_harmonics_%s.csv', state_suffix));
    state_png_file = fullfile(result_dir, sprintf( ...
        '2023a_load_regulation_curve_%s.png', state_suffix));
    state_harmonic_diagnostics_png_file = fullfile(result_dir, sprintf( ...
        ['2023a_load_regulation_harmonic_diagnostics_' ...
         '%s.png'], state_suffix));
    copy_validation_artifact(json_file, state_json_file);
    copy_validation_artifact(csv_file, state_csv_file);
    copy_validation_artifact(harmonics_csv_file, ...
        state_harmonics_csv_file);
    copy_validation_artifact(png_file, state_png_file);
    copy_validation_artifact(harmonic_diagnostics_png_file, ...
        state_harmonic_diagnostics_png_file);
    ...
    fprintf('State-specific outputs: *_%s.*\n', state_suffix);
...
if ~isfile(header_file) || ~isfile(matlab_init_file) || ...
        ~isfile(common_header_file) || ~isfile(common_matlab_init_file)
    error('SINV:R2023A:LoadRegulationGeneratedSettingsMissing', ...
        ['The generated common and simulation C/MATLAB settings files ' ...
         'are all required.']);
end
...
names = {
    ...
    'SINV_2023A_ENABLE_DEADTIME_COMP'
    'CTRL_PWM_DEADBAND_CMP'
    'CTRL_CURRENT_DB_PU'
    'CTRL_CURRENT_BASE'
    'SINV_FILTER_CAPACITANCE_F'
    };

header_text = sprintf('%s\n%s', fileread(common_header_file), ...
    fileread(header_file));
matlab_text = sprintf('%s\n%s', fileread(common_matlab_init_file), ...
    fileread(matlab_init_file));
config = struct;
for k = 1:numel(names)
    name = names{k};
...
if config.SINV_2023A_ENABLE_DEADTIME_COMP ~= 0 && ...
        config.SINV_2023A_ENABLE_DEADTIME_COMP ~= 1
    error('SINV:R2023A:LoadRegulationDeadtimeCompensationInvalid', ...
        'SINV_2023A_ENABLE_DEADTIME_COMP must be 0 or 1.');
end
end
...
function copy_validation_artifact(source_file, destination_file)
[copied, message] = copyfile(source_file, destination_file, 'f');
if ~copied
    error('SINV:R2023A:LoadRegulationArtifactCopyFailed', ...
        'Could not copy %s to %s: %s', ...
        source_file, destination_file, message);
end
if ~isequal(read_binary_file(source_file), ...
        read_binary_file(destination_file))
    error('SINV:R2023A:LoadRegulationArtifactCopyMismatch', ...
        'State-specific artifact differs from its generic source: %s', ...
        destination_file);
end
end


function bytes = read_binary_file(filename)
fid = fopen(filename, 'rb');
if fid < 0
    error('SINV:R2023A:LoadRegulationArtifactReadFailed', ...
        'Could not read validation artifact: %s', filename);
end
file_cleanup = onCleanup(@() close_file_quietly(fid)); %#ok<NASGU>
...
function value = enabled_text(flag)
if flag
    value = 'enabled';
else
    value = 'disabled';
