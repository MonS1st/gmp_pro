...
try
    if ~isfile(model_file)
        error('SINV:2023A:LoadRegulationModelMissing', ...
            'The fixed 2023A load-regulation model does not exist: %s', ...
            model_file);
    end
    if ~isfile(exe)
        error('SINV:2023A:LoadRegulationExecutableMissing', ...
    ...
    if abs(config.SINV_2023A_UO_REF_RMS_V - target_voltage_rms_v) > 1e-12
        error('SINV:2023A:LoadRegulationVoltageTargetMismatch', ...
    ...
    if stop_time < minimum_end_time
        error('SINV:2023A:LoadRegulationStopTimeTooShort', ...
    ...
    if bdIsLoaded(model)
        error('SINV:2023A:LoadRegulationModelAlreadyLoaded', ...
    ...
    for point_index = 1:point_count
        ...
        if ~override_confirmed
            error('SINV:2023A:LoadOverrideIneffective', ...
        ...
        if ~isempty(missing_point_fields) || ~isempty(extra_point_fields)
            error('SINV:2023A:LoadRegulationPointSchemaMismatch', ...
    ...
    if ~model_file_unchanged
        error('SINV:2023A:LoadRegulationModelFileChanged', ...
    ...
    if ~all_controller_processes_stopped
        error('SINV:2023A:LoadRegulationControllerStillRunning', ...
    ...
    if ~passed
        print_failures(point_results, overall_checks, ...
            endpoint_load_regulation_percent, ...
            span_load_regulation_percent);
        error('SINV:2023A:LoadRegulationValidationFailed', ...
...
if analysis_cycles < minimum_analysis_cycles
    error('SINV:2023A:LoadRegulationSteadyWindowTooShort', ...
...
if controller.HasExited
    error('SINV:2023A:LoadRegulationControllerExitedEarly', ...
...
if ~isfile(header_file) || ~isfile(matlab_init_file)
    error('SINV:2023A:LoadRegulationGeneratedSettingsMissing', ...
...
for k = 1:numel(names)
    ...
    if abs(c_value - matlab_value) > tolerance
        error('SINV:2023A:LoadRegulationGeneratedSettingsDisagree', ...
...
if config.BUILD_LEVEL ~= expected_build_level || ...
    ...
    error('SINV:2023A:LoadRegulationGeneratedModeMismatch', ...
...
if numel(tokens) ~= 1
    error('SINV:2023A:LoadRegulationMacroMissingOrDuplicate', ...
        'Expected exactly one generated C macro named %s.', name);
end
value = str2double(tokens{1}{1});
if ~isfinite(value)
    error('SINV:2023A:LoadRegulationMacroInvalid', ...
...
tokens = regexp(text, expression, 'tokens');
if numel(tokens) ~= 1
    error('SINV:2023A:LoadRegulationVariableMissingOrDuplicate', ...
        'Expected exactly one generated MATLAB variable named %s.', name);
end
value = str2double(tokens{1}{1});
if ~isfinite(value)
    error('SINV:2023A:LoadRegulationVariableInvalid', ...
...
if getSimulinkBlockHandle(load_block) == -1
    error('SINV:2023A:LoadRegulationLoadBlockMissing', ...
...
if ~strcmp(resistance_expression, 'SINV_RLOAD_OHM')
    error('SINV:2023A:LoadRegulationLoadExpressionMismatch', ...
...
blocks = find_system(model, ...
    ...
    'Name', block_name);
if numel(blocks) ~= 1
    error('SINV:2023A:LoadRegulationPhysicalLoggerMissing', ...
...
if ~strcmp(get_param(blocks{1}, 'BlockType'), 'ToWorkspace') || ...
    ...
    error('SINV:2023A:LoadRegulationPhysicalLoggerInvalid', ...
...
if getSimulinkBlockHandle(source) == -1
    error('SINV:2023A:LoadRegulationMonitorSourceMissing', ...
...
if getSimulinkBlockHandle(block) ~= -1
    error('SINV:2023A:LoadRegulationTemporaryLoggerExists', ...
...
if numel(source_handles.Outport) < source_port
    error('SINV:2023A:LoadRegulationMonitorPortMissing', ...
...
if getSimulinkBlockHandle(constant_block) ~= -1 || ...
    ...
    error('SINV:2023A:LoadRegulationProbeExists', ...
...
if isempty(process)
    error('SINV:2023A:LoadRegulationControllerStartFailed', ...
...
if ~process.WaitForExit(5000)
    error('SINV:2023A:LoadRegulationControllerStopTimeout', ...
...
try
    ...
    emit_cleanup_warning( ...
        'SINV:2023A:LoadRegulationControllerCleanupFailed', ...
...
error('SINV:2023A:LoadRegulationControllerAlreadyRunning', ...
...
for index = 1:process_count
    try
        ...
        error('SINV:2023A:LoadRegulationProcessEnumerationAccessFailed', ...
            ['Could not access .NET process-array element %d of %d using ' ...
             'GetValue(%d): %s'], ...
            index, process_count, index - 1, access_error.message);
    end
...
try
    ...
    emit_cleanup_warning('SINV:2023A:LoadRegulationModelCleanupFailed', ...
...
try
    ...
    emit_cleanup_warning( ...
        'SINV:2023A:LoadRegulationCleanupReportFailedSafely', ...
...
try
    if isfile(model_file) && ~isempty(model_info_before)
        ...
        if model_info_after.bytes ~= model_info_before.bytes || ...
                model_info_after.datenum ~= model_info_before.datenum
            emit_cleanup_warning( ...
                'SINV:2023A:LoadRegulationModelFileChangedDuringError', ...
    ...
    emit_cleanup_warning( ...
        'SINV:2023A:LoadRegulationModelCleanupStateUnavailable', ...
...
try
    ...
    emit_cleanup_warning( ...
        'SINV:2023A:LoadRegulationProcessEnumerationFailed', ...
...
if isempty(process_list)
    ...
    emit_cleanup_warning( ...
        'SINV:2023A:LoadRegulationControllerRemainsDuringError', ...
...
try
    ...
    error('SINV:2023A:LoadRegulationSignalMissing', ...
        'SimulationOutput does not contain %s: %s', name, ME.message);
end
if isempty(raw)
    error('SINV:2023A:LoadRegulationSignalEmpty', ...
...
if numel(time) < 2 || time(end) <= time(1)
    error('SINV:2023A:LoadRegulationSignalTimeInvalid', ...
...
if end_time <= start_time
    error('SINV:2023A:LoadRegulationFrequencyWindowInvalid', ...
...
if numel(crossing_index) < 3
    error('SINV:2023A:LoadRegulationFrequencyEstimateFailed', ...
...
if query_time(1) < series.time(1) - tolerance || ...
    ...
    error('SINV:2023A:LoadRegulationSignalWindowUnavailable', ...
...
if any(~isfinite(data))
    error('SINV:2023A:LoadRegulationInterpolationFailed', ...
...
if series.time(1) > start_time + coverage_tolerance_s || ...
    ...
    error('SINV:2023A:LoadRegulationStatusWindowIncomplete', ...
...
if isempty(data)
    error('SINV:2023A:LoadRegulationStatusWindowUnavailable', ...
...
if sample_count ~= cycle_count * samples_per_cycle
    error('SINV:2023A:LoadRegulationHarmonicWindowInvalid', ...
...
if peak_amplitude(1) <= eps(max(abs(data)))
    error('SINV:2023A:LoadRegulationFundamentalMissing', ...
...
if ~isfinite(limit_peak_a) || limit_peak_a <= 0
    error('SINV:2023A:LoadRegulationCurrentLimitInvalid', ...
...
if fid < 0
    error('SINV:2023A:LoadRegulationJSONOpenFailed', ...
